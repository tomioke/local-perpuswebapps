    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id_buku") ?? Route("id_buku")) !== null) {
                ...
                $this->RecKey["id_buku"] = $this->id_buku->QueryStringValue;
            } elseif (Post("id_buku") !== null) {
                $this->id_buku->setFormValue(Post("id_buku"));
                $this->RecKey["id_buku"] = $this->id_buku->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->id_buku->setQueryStringValue($keyValue);
                $this->RecKey["id_buku"] = $this->id_buku->QueryStringValue;
            } else {
                $returnUrl = "BukuList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "BukuList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "BukuList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
