 ...
 * Page class
 */
class PengembalianGrid extends Pengembalian
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PengembalianGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fpengembaliangrid";
    ...
    public function run()
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
                if ($this->getCurrentMasterTable() == "peminjaman") {
                    $this->DbMasterFilter = $this->addMasterUserIDFilter($this->DbMasterFilter, "peminjaman"); // Add master User ID filter
                }
        }
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "peminjaman") {
            $masterTbl = Container("peminjaman");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("PeminjamanList"); // Return to master page
                return;
            } else {
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->id_kembali->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_id_peminjaman") && $CurrentForm->hasValue("o_id_peminjaman") && $this->id_peminjaman->CurrentValue != $this->id_peminjaman->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_tgl_kembali") && $CurrentForm->hasValue("o_tgl_kembali") && $this->tgl_kembali->CurrentValue != $this->tgl_kembali->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_kondisi_buku_kembali") && $CurrentForm->hasValue("o_kondisi_buku_kembali") && $this->kondisi_buku_kembali->CurrentValue != $this->kondisi_buku_kembali->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->id_kembali->clearErrorMessage();
        $this->id_peminjaman->clearErrorMessage();
        $this->tgl_kembali->clearErrorMessage();
        $this->kondisi_buku_kembali->clearErrorMessage();
    }
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetall") {
                ...
                $this->DbMasterFilter = "";
                $this->DbDetailFilter = "";
                        $this->id_peminjaman->setSessionValue("");
            }
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex) && $this->RowAction != "delete") {
            if ($keyName != "") {
                $this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $keyName . "\" id=\"" . $keyName . "\" value=\"" . $this->id_kembali->CurrentValue . "\">";
            }
        }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_kembali->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
        ...
        if ($CurrentForm->hasValue("o_id_peminjaman")) {
            $this->id_peminjaman->setOldValue($CurrentForm->getValue("o_id_peminjaman"));
        }
        ...
        if ($CurrentForm->hasValue("o_tgl_kembali")) {
            $this->tgl_kembali->setOldValue($CurrentForm->getValue("o_tgl_kembali"));
        }
        ...
        if ($CurrentForm->hasValue("o_kondisi_buku_kembali")) {
            $this->kondisi_buku_kembali->setOldValue($CurrentForm->getValue("o_kondisi_buku_kembali"));
        }
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->id_peminjaman->getSessionValue() != "") {
                $this->id_peminjaman->CurrentValue = GetForeignKeyValue($this->id_peminjaman->getSessionValue());
                $this->id_peminjaman->OldValue = $this->id_peminjaman->CurrentValue;
                $curVal = strval($this->id_peminjaman->CurrentValue);
                if ($curVal != "") {
                    $this->id_peminjaman->ViewValue = $this->id_peminjaman->lookupCacheOption($curVal);
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
            $masterFilter = $this->sqlMasterFilter_peminjaman();
            if (strval($this->id_peminjaman->CurrentValue) != "") {
                $masterFilter = str_replace("@id_peminjaman@", AdjustSql($this->id_peminjaman->CurrentValue, "DB"), $masterFilter);
            } else {
                $masterFilter = "";
            }
            if ($masterFilter != "") {
                $rsmaster = Container("peminjaman")->loadRs($masterFilter)->fetch(\PDO::FETCH_ASSOC);
                $this->MasterRecordExists = $rsmaster !== false;
                $validMasterKey = true;
                if ($this->MasterRecordExists) {
                    $validMasterKey = $Security->isValidUserID($rsmaster['id_anggota']);
                } elseif ($this->getCurrentMasterTable() == "peminjaman") {
        ...
        if ($this->getCurrentMasterTable() == "peminjaman") {
            $this->id_peminjaman->CurrentValue = $this->id_peminjaman->getSessionValue();
        }
        $conn = $this->getConnection();
