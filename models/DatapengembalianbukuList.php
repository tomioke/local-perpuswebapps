 ...
 * Page class
 */
class DatapengembalianbukuList extends Datapengembalianbuku
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'data-pengembalian-buku';

    // Page object name
    public $PageObjName = "DatapengembalianbukuList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fdatapengembalianbukulist";
    ...
    public function __construct()
    {
        ...
        // Table object (datapengembalianbuku)
        if (!isset($GLOBALS["datapengembalianbuku"]) || get_class($GLOBALS["datapengembalianbuku"]) == PROJECT_NAMESPACE . "datapengembalianbuku") {
            $GLOBALS["datapengembalianbuku"] = &$this;
        }
        ...
        $this->AddUrl = "DatapengembalianbukuAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "DatapengembalianbukuDelete";
        $this->MultiUpdateUrl = "DatapengembalianbukuUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'data-pengembalian-buku');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fdatapengembalianbukulistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("datapengembalianbuku"));
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fdatapengembalianbukulistsrch", $filters);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->berita_peminjaman); // berita_peminjaman
            $this->updateSort($this->id_buku); // id_buku
            $this->updateSort($this->id_anggota); // id_anggota
            $this->updateSort($this->tgl_peminjaman); // tgl_peminjaman
            $this->updateSort($this->rencana_tgl_kembali); // rencana_tgl_kembali
            $this->updateSort($this->kondisi_buku_peminjaman); // kondisi_buku_peminjaman
            $this->updateSort($this->tgl_kembali); // tgl_kembali
            $this->updateSort($this->kondisi_buku_kembali); // kondisi_buku_kembali
            $this->updateSort($this->Lama_Kembali); // Lama_Kembali
            $this->updateSort($this->Lama_Pinjam); // Lama_Pinjam
            $this->updateSort($this->Terlambat); // Terlambat
            $this->updateSort($this->id_kembali); // id_kembali
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->id_kembali->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdatapengembalianbukulistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdatapengembalianbukulistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fdatapengembalianbukulist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->id_buku->ViewValue = FormatNumber($this->id_buku->ViewValue, 0, -2, -2, -2);
            $this->id_buku->ViewCustomAttributes = "";

            // id_anggota
            $this->id_anggota->ViewValue = $this->id_anggota->CurrentValue;
            $this->id_anggota->ViewValue = FormatNumber($this->id_anggota->ViewValue, 0, -2, -2, -2);
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fdatapengembalianbukulistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
