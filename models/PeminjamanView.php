 ...
 * Page class
 */
class PeminjamanView extends Peminjaman
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PeminjamanView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("id_peminjaman") ?? Route("id_peminjaman")) !== null) {
            $this->RecKey["id_peminjaman"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id_peminjaman") ?? Route("id_peminjaman")) !== null) {
                ...
                $this->RecKey["id_peminjaman"] = $this->id_peminjaman->QueryStringValue;
            } elseif (Post("id_peminjaman") !== null) {
                $this->id_peminjaman->setFormValue(Post("id_peminjaman"));
                $this->RecKey["id_peminjaman"] = $this->id_peminjaman->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->id_peminjaman->setQueryStringValue($keyValue);
                $this->RecKey["id_peminjaman"] = $this->id_peminjaman->QueryStringValue;
            } else {
                $returnUrl = "PeminjamanList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "PeminjamanList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "PeminjamanList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
