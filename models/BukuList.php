...
class BukuList extends Buku
{
    ...
    // Page object name
    public $PageObjName = "BukuList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fbukulist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "BukuDelete";
        $this->MultiUpdateUrl = "BukuUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fbukulistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->nama_buku->AdvancedSearch->toJson(), ","); // Field nama_buku
        $filterList = Concat($filterList, $this->pengarang->AdvancedSearch->toJson(), ","); // Field pengarang
        $filterList = Concat($filterList, $this->penerbit->AdvancedSearch->toJson(), ","); // Field penerbit
        $filterList = Concat($filterList, $this->kode_isbn->AdvancedSearch->toJson(), ","); // Field kode_isbn
        $filterList = Concat($filterList, $this->rangkuman->AdvancedSearch->toJson(), ","); // Field rangkuman
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fbukulistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field nama_buku
        $this->nama_buku->AdvancedSearch->SearchValue = @$filter["x_nama_buku"];
        $this->nama_buku->AdvancedSearch->SearchOperator = @$filter["z_nama_buku"];
        $this->nama_buku->AdvancedSearch->SearchCondition = @$filter["v_nama_buku"];
        $this->nama_buku->AdvancedSearch->SearchValue2 = @$filter["y_nama_buku"];
        $this->nama_buku->AdvancedSearch->SearchOperator2 = @$filter["w_nama_buku"];
        $this->nama_buku->AdvancedSearch->save();

        // Field pengarang
        $this->pengarang->AdvancedSearch->SearchValue = @$filter["x_pengarang"];
        $this->pengarang->AdvancedSearch->SearchOperator = @$filter["z_pengarang"];
        $this->pengarang->AdvancedSearch->SearchCondition = @$filter["v_pengarang"];
        $this->pengarang->AdvancedSearch->SearchValue2 = @$filter["y_pengarang"];
        $this->pengarang->AdvancedSearch->SearchOperator2 = @$filter["w_pengarang"];
        $this->pengarang->AdvancedSearch->save();

        // Field penerbit
        $this->penerbit->AdvancedSearch->SearchValue = @$filter["x_penerbit"];
        $this->penerbit->AdvancedSearch->SearchOperator = @$filter["z_penerbit"];
        $this->penerbit->AdvancedSearch->SearchCondition = @$filter["v_penerbit"];
        $this->penerbit->AdvancedSearch->SearchValue2 = @$filter["y_penerbit"];
        $this->penerbit->AdvancedSearch->SearchOperator2 = @$filter["w_penerbit"];
        $this->penerbit->AdvancedSearch->save();

        // Field kode_isbn
        $this->kode_isbn->AdvancedSearch->SearchValue = @$filter["x_kode_isbn"];
        $this->kode_isbn->AdvancedSearch->SearchOperator = @$filter["z_kode_isbn"];
        $this->kode_isbn->AdvancedSearch->SearchCondition = @$filter["v_kode_isbn"];
        $this->kode_isbn->AdvancedSearch->SearchValue2 = @$filter["y_kode_isbn"];
        $this->kode_isbn->AdvancedSearch->SearchOperator2 = @$filter["w_kode_isbn"];
        $this->kode_isbn->AdvancedSearch->save();

        // Field rangkuman
        $this->rangkuman->AdvancedSearch->SearchValue = @$filter["x_rangkuman"];
        $this->rangkuman->AdvancedSearch->SearchOperator = @$filter["z_rangkuman"];
        $this->rangkuman->AdvancedSearch->SearchCondition = @$filter["v_rangkuman"];
        $this->rangkuman->AdvancedSearch->SearchValue2 = @$filter["y_rangkuman"];
        $this->rangkuman->AdvancedSearch->SearchOperator2 = @$filter["w_rangkuman"];
        $this->rangkuman->AdvancedSearch->save();
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->cover, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->nama_buku, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->kode_isbn, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->rangkuman, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->nama_buku, $ctrl); // nama_buku
            $this->updateSort($this->pengarang, $ctrl); // pengarang
            $this->updateSort($this->penerbit, $ctrl); // penerbit
            $this->updateSort($this->kode_isbn, $ctrl); // kode_isbn
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->nama_buku->setSort("");
                $this->pengarang->setSort("");
                $this->penerbit->setSort("");
                $this->kode_isbn->setSort("");
                $this->rangkuman->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->id_buku->CurrentValue) . "\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbukulistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbukulistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fbukulist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fbukulist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fbukulist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fbukulist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_buku" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_buku\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fbukulist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fbukulistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "penerbit") {
            $penerbit = Container("penerbit");
            $rsmaster = $penerbit->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $penerbit;
                    $penerbit->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
        ...
        // Export master record
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "pengarang") {
            $pengarang = Container("pengarang");
            $rsmaster = $pengarang->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $pengarang;
                    $pengarang->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
                $doc->setStyle($exportStyle); // Restore
