 ...
 */
class Peminjaman extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // id_peminjaman
        $this->id_peminjaman = new DbField('peminjaman', 'peminjaman', 'x_id_peminjaman', 'id_peminjaman', '`id_peminjaman`', '`id_peminjaman`', 3, 100, -1, false, '`id_peminjaman`', false, false, false, 'FORMATTED TEXT', 'NO');
        $this->id_peminjaman->IsAutoIncrement = true; // Autoincrement field
        ...
        // berita_peminjaman
        $this->berita_peminjaman = new DbField('peminjaman', 'peminjaman', 'x_berita_peminjaman', 'berita_peminjaman', '`berita_peminjaman`', '`berita_peminjaman`', 200, 200, -1, false, '`berita_peminjaman`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // id_buku
        $this->id_buku = new DbField('peminjaman', 'peminjaman', 'x_id_buku', 'id_buku', '`id_buku`', '`id_buku`', 3, 100, -1, false, '`EV__id_buku`', true, true, true, 'FORMATTED TEXT', 'SELECT');
        ...
        // id_anggota
        $this->id_anggota = new DbField('peminjaman', 'peminjaman', 'x_id_anggota', 'id_anggota', '`id_anggota`', '`id_anggota`', 3, 100, -1, false, '`EV__id_anggota`', true, true, true, 'FORMATTED TEXT', 'SELECT');
        ...
        // tgl_peminjaman
        $this->tgl_peminjaman = new DbField('peminjaman', 'peminjaman', 'x_tgl_peminjaman', 'tgl_peminjaman', '`tgl_peminjaman`', CastDateFieldForLike("`tgl_peminjaman`", 0, "DB"), 133, 10, 0, false, '`tgl_peminjaman`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // rencana_tgl_kembali
        $this->rencana_tgl_kembali = new DbField('peminjaman', 'peminjaman', 'x_rencana_tgl_kembali', 'rencana_tgl_kembali', '`rencana_tgl_kembali`', CastDateFieldForLike("`rencana_tgl_kembali`", 0, "DB"), 133, 10, 0, false, '`rencana_tgl_kembali`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        $this->kondisi_buku_peminjaman = new DbField('peminjaman', 'peminjaman', 'x_kondisi_buku_peminjaman', 'kondisi_buku_peminjaman', '`kondisi_buku_peminjaman`', '`kondisi_buku_peminjaman`', 200, 200, -1, false, '`kondisi_buku_peminjaman`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`peminjaman`";
    }
    ...
    public function getSqlSelectList() // Select for List page
    {
        ...
        $from = "(SELECT *, (SELECT `nama_buku` FROM `buku` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`id_buku` = `peminjaman`.`id_buku` LIMIT 1) AS `EV__id_buku`, (SELECT `nama_anggota` FROM `anggota` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`id_anggota` = `peminjaman`.`id_anggota` LIMIT 1) AS `EV__id_anggota` FROM `peminjaman`)";
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->id_peminjaman->setDbValue($conn->lastInsertId());
            $rs['id_peminjaman'] = $this->id_peminjaman->DbValue;
        }
        return $success;
    }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('id_peminjaman', $rs)) {
                AddFilter($where, QuotedName('id_peminjaman', $this->Dbid) . '=' . QuotedValue($rs['id_peminjaman'], $this->id_peminjaman->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        return "`id_peminjaman` = @id_peminjaman@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_peminjaman->CurrentValue : $this->id_peminjaman->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_peminjaman->CurrentValue = $keys[0];
            } else {
                $this->id_peminjaman->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_peminjaman', $row) ? $row['id_peminjaman'] : null;
        } else {
            $val = $this->id_peminjaman->OldValue !== null ? $this->id_peminjaman->OldValue : $this->id_peminjaman->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_peminjaman@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "PeminjamanEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "PeminjamanAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_ADD_ACTION"):
                return "PeminjamanAdd";
            case Config("API_EDIT_ACTION"):
                return "PeminjamanEdit";
            case Config("API_DELETE_ACTION"):
                return "PeminjamanDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("PeminjamanView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("PeminjamanView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "PeminjamanAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "PeminjamanAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("PeminjamanEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("PeminjamanAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("PeminjamanDelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "id_peminjaman:" . JsonEncode($this->id_peminjaman->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_peminjaman->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->id_peminjaman->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("id_peminjaman") ?? Route("id_peminjaman")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->id_peminjaman->CurrentValue = $key;
            } else {
                $this->id_peminjaman->OldValue = $key;
            }
    ...
    public function getUserIDSubquery(&$fld, &$masterfld)
    {
        global $UserTable;
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM `peminjaman`";
