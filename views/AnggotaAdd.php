...
loadjs.ready("head", function () {
    ...
    fanggotaadd = currentForm = new ew.Form("fanggotaadd", "add");
    ...
    fanggotaadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fanggotaadd.lists.id_level = <?= $Page->id_level->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fanggotaadd" id="fanggotaadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<script>
loadjs.ready(["fanggotaadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fanggotaadd", "x_tgl_lahir", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<div class="input-group" id="ig__password">
    <input type="password" autocomplete="new-password" data-table="anggota" data-field="x__password" name="x__password" id="x__password" size="30" maxlength="200" placeholder="<?= HtmlEncode($Page->_password->getPlaceHolder()) ?>"<?= $Page->_password->editAttributes() ?> aria-describedby="x__password_help">
