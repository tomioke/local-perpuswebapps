...
loadjs.ready("head", function () {
    ...
    // Validate form
    fpengembalianedit.validate = function () {
    ...
    // Form_CustomValidate
    fpengembalianedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpengembalianedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpengembalianedit.lists.id_peminjaman = <?= $Page->id_peminjaman->toClientList($Page) ?>;
    loadjs.done("fpengembalianedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpengembalianedit" id="fpengembalianedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id_kembali->Visible) { // id_kembali ?>
    <div id="r_id_kembali" class="form-group row">
        <label id="elh_pengembalian_id_kembali" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_kembali->caption() ?><?= $Page->id_kembali->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_kembali->getDisplayValue($Page->id_kembali->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pengembalian" data-field="x_id_kembali" data-hidden="1" name="x_id_kembali" id="x_id_kembali" value="<?= HtmlEncode($Page->id_kembali->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fpengembalianedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fpengembalianedit", "x_tgl_kembali", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
