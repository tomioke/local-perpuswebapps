...
loadjs.ready("head", function () {
    ...
    fbukuedit = currentForm = new ew.Form("fbukuedit", "edit");
    ...
    // Form_CustomValidate
    fbukuedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fbukuedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fbukuedit.lists.pengarang = <?= $Page->pengarang->toClientList($Page) ?>;
    fbukuedit.lists.penerbit = <?= $Page->penerbit->toClientList($Page) ?>;
    loadjs.done("fbukuedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fbukuedit" id="fbukuedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id_buku->Visible) { // id_buku ?>
    <div id="r_id_buku" class="form-group row">
        <label id="elh_buku_id_buku" for="x_id_buku" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_buku->caption() ?><?= $Page->id_buku->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_buku->cellAttributes() ?>>
<span id="el_buku_id_buku">
<span<?= $Page->id_buku->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_buku->getDisplayValue($Page->id_buku->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="buku" data-field="x_id_buku" data-hidden="1" name="x_id_buku" id="x_id_buku" value="<?= HtmlEncode($Page->id_buku->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fbukuedit", "editor"], function() {
	ew.createEditor("fbukuedit", "x_rangkuman", 35, 4, <?= $Page->rangkuman->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
