...
loadjs.ready("head", function () {
    ...
    fpeminjamanadd = currentForm = new ew.Form("fpeminjamanadd", "add");

    // Add fields
    var fields = ew.vars.tables.peminjaman.fields;
    fpeminjamanadd.addFields([
    ...
    // Validate form
    fpeminjamanadd.validate = function () {
    ...
    // Form_CustomValidate
    fpeminjamanadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpeminjamanadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpeminjamanadd.lists.id_buku = <?= $Page->id_buku->toClientList($Page) ?>;
    fpeminjamanadd.lists.id_anggota = <?= $Page->id_anggota->toClientList($Page) ?>;
    loadjs.done("fpeminjamanadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpeminjamanadd" id="fpeminjamanadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<script>
loadjs.ready(["fpeminjamanadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fpeminjamanadd", "x_rencana_tgl_kembali", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
